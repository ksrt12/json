...
const keys = (o: ILogAppFull) => Object.keys(o);
const len = (o: ILogAppFull) => keys(o).length;

const MainPage: React.FC = () => {
    const [disDiff, setDisDiff] = useState(true);
    const [diff, setDiff] = useState(false);
    const [filesList, setFilesList] = useState([] as ILogAppFull[]);
    const [finalData, setMergedData] = useState({} as ILogAppFull);
    ...
    const parseJSON = (str: string) => {
        const preresult: ILogAppFull = JSON.parse(str);
        setFilesList(prev => [preresult, ...prev]);
    };

    const mergeJSONs = useCallback((/*filesList: IJson[]*/) => {
        let dubl = "";
        let dublJson: ILogAppFull = {};
        let mergedData: ILogAppFull = {};
    ...
    const json2csv = useCallback(() => {
        ...
        csvBtn.update(`${diff ? "Diff" : "Merged"}.csv`, akt2csv(csv));
